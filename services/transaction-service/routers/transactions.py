...
@router.post("/", response_model=TransactionResponse, dependencies=[Depends(auth_staff)])
async def create_transaction(transaction: TransactionCreate, request: Request, db=Depends(get_database)):
    headers = get_identity_headers(request)
    
    # 1. Validate vehicle exists
    if not await validate_vehicle(transaction.vehicleId, headers=headers):
    ...
    if not await validate_fuel_stock(transaction.stationId, transaction.fuelType, transaction.litersServed, headers=headers):
        raise HTTPException(status_code=400, detail="Insufficient Fuel Stock at station")

    # 3. Deduct consumed liters from FuelStock
    if not await update_fuel_stock(transaction.stationId, transaction.fuelType, transaction.litersServed, headers=headers):
        raise HTTPException(status_code=500, detail="Failed to update fuel stock")

    # 4. Deduct consumed liters from Quota
    if not await update_vehicle_quota(transaction.vehicleId, transaction.litersServed, headers=headers):
        # Rollback fuel stock if quota update fails (simplified rollback)
        await update_fuel_stock(transaction.stationId, transaction.fuelType, -transaction.litersServed, headers=headers)
        raise HTTPException(status_code=500, detail="Failed to update vehicle quota")

    # 5. Update Queue status to served if applicable
    await update_queue_status(transaction.vehicleId, transaction.stationId, headers=headers)
...
@router.get("/", response_model=List[TransactionResponse], dependencies=[Depends(auth_admin)])
...
@router.get("/{id}", response_model=TransactionResponse, dependencies=[Depends(auth_all)])
...
@router.put("/{id}", response_model=TransactionResponse, dependencies=[Depends(auth_staff)])
...
@router.get("/station/{stationId}", response_model=List[TransactionResponse], dependencies=[Depends(auth_all)])
...
@router.get("/vehicle/{vehicleId}", response_model=List[TransactionResponse], dependencies=[Depends(auth_all)])
